...
import de.unitrier.st.insituprofiling.core.data.GlobalResetThreadArtifactFilter;
import de.unitrier.st.insituprofiling.core.data.ProfilingArtifactMetricValueSelfComparator;
...
import de.unitrier.st.insituprofiling.core.visualization.popup.MetricTableCellRenderer;
import de.unitrier.st.insituprofiling.core.visualization.popup.MetricTableMouseMotionAdapter;
...
public class ProfilingArtifactOverview
{
    private static volatile ProfilingArtifactOverview instance;

    public static ProfilingArtifactOverview getInstance()
    {
        if (instance == null)
        {
            synchronized (ProfilingArtifactOverview.class)
            {
                if (instance == null)
                {
                    instance = new ProfilingArtifactOverview();
                }
            }
        }
        return instance;
    ...
    private IProfilingResult result;

    public void setProfilingResult(final IProfilingResult result)
    {
        if (result == null)
        {
            return;
    ...
    private ProfilingArtifactOverview()
    {
        setupUI();
    }
...
//        {
        ...
        resetThreadFilterButton.addActionListener(e -> {
            ...
            ProfilingFlowManager.getInstance().getCurrentProfilingFlow().applyThreadArtifactFilter(GlobalResetThreadArtifactFilter
                    .getInstance());
        });
...
//        }
    ...
    public void filterOverView()
    {
        ...
        ApplicationManager.getApplication().invokeLater(() -> {
            ...
            Map<String, List<AProfilingArtifact>> lists = result.getNamedArtifactTypeLists();

            ProfilingArtifactMetricValueSelfComparator profilingArtifactMetricValueSelfComparator =
                    new ProfilingArtifactMetricValueSelfComparator();

            tabbedPane.removeChangeListener(tabbedPaneChangeListener);

            clear();

            for (Map.Entry<String, List<AProfilingArtifact>> entry : lists.entrySet())
            {
                List<AProfilingArtifact> artifacts = entry.getValue();
                artifacts = filterArtifacts(artifacts, includeFilters, excludeFilters);
                artifacts.sort(profilingArtifactMetricValueSelfComparator);
...
//                    UserActivityLogger.getInstance().log(UserActivityEnum.OverviewClassesTabEntered);
    ...
    private void addTab(String title, List<AProfilingArtifact> artifacts)
    {
        final ProfilingArtifactOverViewTableModel tableModel = new ProfilingArtifactOverViewTableModel(artifacts);
        final MetricTable jbTable = new MetricTable(tableModel)
        {
            ...
            public String getToolTipText(@NotNull MouseEvent e)
            {
                ...
                AProfilingArtifact artifactAt = tableModel.getArtifactAt(rowIndex);
                if (artifactAt == null)
                {
                    return "";
                }
        ...
        jbTable.addMouseListener(new ProfilingArtifactOverviewTableMouseAdapter(jbTable));
...
//        threadStateFilterWrapper.add(threadStatesGrid, BorderLayout.CENTER);
    ...
    private List<AProfilingArtifact> filterArtifacts(Collection<? extends AProfilingArtifact> artifacts, Set<String> includeElements,
                                                     Set<String> excludeElements)
    {
        List<AProfilingArtifact> filtered = new ArrayList<>();
        if (artifacts == null)
        {
            return filtered;
        }
        ...
        if (currentFileFilterSelected)
        {
            if (currentFileArtifactFilter == null)
            {
                ProfilingLogger.addText(String.format("%s: current file artifact filter not setup!", getClass()));
            } else
            {
        ...
        if (standardLibraryFilter.isSelected())
        {
            if (standardLibraryArtifactFilter == null)
            {
                ProfilingLogger.addText(String.format("%s: standard library artifact filter not setup!", getClass()));
            } else
            {
