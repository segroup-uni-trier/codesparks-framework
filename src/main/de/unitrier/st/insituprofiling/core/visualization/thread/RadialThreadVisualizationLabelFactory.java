...
import static de.unitrier.st.insituprofiling.core.visualization.thread.RadialThreadVisualizationConstants.*;
...
public class RadialThreadVisualizationLabelFactory extends AArtifactVisualizationLabelFactory
{
    ...
    public JLabel createArtifactLabel(@NotNull AProfilingArtifact artifact)
    {
        ...
        final int frameSize = FRAME + completeNumberOfThreadsString.length() * 5;
        final int labelWidth = 5 + completeNumberOfThreadsString.length() * 5;

        GraphicsConfiguration defaultConfiguration =
                GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration();
        BufferedImage bi = UIUtil.createImage(defaultConfiguration, frameSize, CIRCLE_FRAMESIZE,
        ...
        for (int i = 0; i < threadArtifactClusters.size(); i++)
        {
            ...
            int radius = ThreadVisualizationUtil.metricToDiscreteMetric(filteredRuntimeRatio, CIRCLESIZE);
            int radiusSum = ThreadVisualizationUtil.metricToDiscreteMetric(filteredRuntimeRatioSum, CIRCLESIZE);
            ...
            g2d.fillArc(MIDDLEPOINT - (radiusSum / 2), MIDDLEPOINT - (radiusSum / 2), radiusSum, radiusSum, startAngle, angle);
            g2d.setColor(color);
            g2d.fillArc(MIDDLEPOINT - (radius / 2), MIDDLEPOINT - (radius / 2), radius, radius, startAngle, angle);
        ...
        g2d.drawOval((CIRCLE_FRAMESIZE / 2) - (CIRCLESIZE / 2), (CIRCLE_FRAMESIZE / 2) - (CIRCLESIZE / 2), CIRCLESIZE, CIRCLESIZE);


        // draw total number of threads label
        Font currentFont = g2d.getFont();
        Font newFont = currentFont.deriveFont(currentFont.getSize() * CIRCLESIZE * 0.02f);
        g2d.setFont(newFont);
        int labelStartAngle = (int) (ThreadVisualizationUtil.getStartAngle(RADIUS, LABELRADIUS) * -1);//calcStartAngle() * -1; //-65
        int arcAngle = 32;
        int x1 = (int) ((LABELRADIUS) * Math.cos(Math.toRadians(-labelStartAngle - arcAngle))) + CIRCLE_FRAMESIZE / 2;
        int y1 = (int) ((LABELRADIUS) * Math.sin(Math.toRadians(-labelStartAngle - arcAngle))) + CIRCLE_FRAMESIZE / 2;
        int x2 = x1 + labelWidth;
        int y2 = y1;

        int x3 = (int) (LABELRADIUS * Math.cos(Math.toRadians(-labelStartAngle))) + CIRCLE_FRAMESIZE / 2;
        int y3 = (int) (LABELRADIUS * Math.sin(Math.toRadians(-labelStartAngle))) + CIRCLE_FRAMESIZE / 2;
        ...
        g2d.drawArc((CIRCLE_FRAMESIZE / 2) - ((CIRCLESIZE + 2 * LABELDISTANCE) / 2),
                (CIRCLE_FRAMESIZE / 2) - ((CIRCLESIZE + 2 * LABELDISTANCE) / 2), CIRCLESIZE + 2 * LABELDISTANCE,
                CIRCLESIZE + 2 * LABELDISTANCE, labelStartAngle, arcAngle);

        g2d.drawString(numberOfSelectedArtifactThreads + "", x1 + 2, y3 - 1);

        // draw number of different classes label
        labelStartAngle = (int) ThreadVisualizationUtil.getStartAngle(RADIUS, LABELRADIUS);
        x1 = (int) ((LABELRADIUS) * Math.cos(Math.toRadians(-(labelStartAngle - arcAngle)))) + CIRCLE_FRAMESIZE / 2;
        y1 = (int) ((LABELRADIUS) * Math.sin(Math.toRadians(-(labelStartAngle - arcAngle)))) + CIRCLE_FRAMESIZE / 2;
        x2 = x1 + labelWidth;
        y2 = y1;

        x3 = (int) (LABELRADIUS * Math.cos(Math.toRadians(-labelStartAngle))) + CIRCLE_FRAMESIZE / 2;
        y3 = (int) (LABELRADIUS * Math.sin(Math.toRadians(-labelStartAngle))) + CIRCLE_FRAMESIZE / 2;
        ...
        g2d.drawArc((CIRCLE_FRAMESIZE / 2) - ((CIRCLESIZE + 2 * LABELDISTANCE) / 2),
                (CIRCLE_FRAMESIZE / 2) - ((CIRCLESIZE + 2 * LABELDISTANCE) / 2), CIRCLESIZE + 2 * LABELDISTANCE,
                CIRCLESIZE + 2 * LABELDISTANCE, labelStartAngle - arcAngle, arcAngle - 4);
